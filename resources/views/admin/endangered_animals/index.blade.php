<x-app-layout>
    <div class="flex min-h-screen bg-gray-100 relative">
        ...
        <div class="w-64 bg-[#38761D] text-white p-4 shadow-lg flex flex-col justify-between">
            <div>
                ...
                <nav class="space-y-2">
                    ...
                    {{-- FIX: Mengganti $item menjadi $menuItem untuk menghindari bentrokan (Walaupun di file ini tidak fatal, ini adalah praktik terbaik) --}}
                    @foreach ($menuItems as $menuItem)
                        @php
                            $href = ($menuItem['route'] == '#') ? '#' : route($menuItem['route']);
                            // Perbaikan: Untuk 'Hewan Langka', aktif jika ada di route 'endangered_animals.*'
                            $isActive = ($menuItem['route'] == 'endangered_animals.index' && request()->routeIs('endangered_animals.*')) || ($menuItem['route'] != '#' && Route::is($menuItem['route']));
                        @endphp
                    ...
                    <div class="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
                        ...
                        <div class="p-4">
                            ...
                            <div class="flex space-x-2 w-full">
                                {{-- Tombol Detail/Edit --}}
                                <a href="{{ route('endangered_animals.show', $animal) }}" class="flex-1 text-center bg-[#6AA84F] hover:bg-[#5C9041] text-white font-bold py-2 px-4 rounded transition duration-150">
                                    Detail
                                </a>

                                {{-- Form Delete --}}
                                <form action="{{ route('endangered_animals.destroy', $animal) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus Hewan Langka {{ $animal->name }}?');">
                                    @csrf
                                    @method('DELETE')
        ...
        <a href="{{ route('endangered_animals.create') }}" class="fixed bottom-8 right-8 bg-[#38761D] hover:bg-[#4A902C] text-white p-4 rounded-full shadow-lg transition duration-300 ease-in-out transform hover:scale-110">
