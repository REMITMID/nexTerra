@php
    use App\Models\Map;
    // Diasumsikan $maps sudah dikirim dari controller atau diambil di sini
    if (!isset($maps)) {
        $maps = Map::all();
    }
@endphp
    ...
    <div class="flex min-h-screen bg-gray-100">
                ...
                <nav class="space-y-2">
                    ...
                    @foreach ($menuItems as $item)
                        @php
                            ...
                            // Cek jika sedang berada di modul Peta
                            $isActive = ($item['route'] == 'map.index' && request()->routeIs('map.*')) || ($item['route'] != '#' && Route::is($item['route']));
                        @endphp
        ...
        <main class="flex-1 p-8 overflow-y-auto">
            ...
            <div class="bg-[#38761D] p-6 rounded-lg shadow-2xl text-white max-w-5xl mx-auto mt-12">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <div class="mb-4">
                                ...
                                <input type="text" id="name" name="name" class="w-full p-2 rounded bg-white border border-gray-600 focus:ring focus:ring-gray-500 text-gray-900" value="{{ old('name', $map->name) }}" placeholder="Contoh: Indonesia">
                        ...
                        @error('image') <span class="text-red-400 text-sm">{{ $message }}</span> @enderror
                            ...
                            <div class="mb-4">
                                ...
                                <textarea id="description" name="description" rows="5" class="w-full p-2 rounded bg-white border border-gray-600 focus:ring focus:ring-gray-500 text-gray-900">{{ old('description', $map->description) }}</textarea>
                        ...
                        <div class="flex flex-col justify-between">
                            <div class="bg-white p-4 rounded flex items-center justify-center text-gray-800 font-bold h-full border border-gray-200 shadow-md flex-grow">
                                @if ($map->image_path)
                                    <img src="{{ asset('storage/' . $map->image_path) }}" alt="{{ $map->name }}" class="w-full h-auto max-h-96 object-contain rounded">
                                @else
                ...
                {{-- Tombol Hapus Peta (Di luar form UPDATE) --}}
                <form action="{{ route('map.destroy', $map) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus Peta ini? Ini juga akan menghapus semua Hewan Langka yang terkait.');" class="mt-4">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-6 rounded transition duration-150">Hapus Peta</button>
                </form>
            </div>
        </main>
    </div>
