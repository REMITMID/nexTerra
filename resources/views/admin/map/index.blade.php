@php
    // Pastikan folder view telah dibuat: resources/views/admin/map
@endphp
<x-app-layout>
    <div class="flex min-h-screen bg-gray-100 relative">
        ...
        <div class="w-64 bg-[#38761D] text-white p-4 shadow-lg flex flex-col justify-between">
            <div>
                ...
                <nav class="space-y-2">
                    ...
                    {{-- FIX: Mengganti $item menjadi $menuItem untuk menghindari bentrokan --}}
                    @foreach ($menuItems as $menuItem)
                        @php
                            $href = ($menuItem['route'] == '#') ? '#' : route($menuItem['route']);
                            $isActive = ($menuItem['route'] != '#') && (Route::is($menuItem['route']) || (str_contains($menuItem['route'], 'map') && request()->routeIs('map.*')));
                        @endphp
        ...
        <a href="{{ route('map.create') }}" class="fixed bottom-8 right-8 bg-[#38761D] hover:bg-[#4A902C] text-white p-4 rounded-full shadow-lg transition duration-300 ease-in-out transform hover:scale-110">
