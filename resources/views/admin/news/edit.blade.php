    ...
    <div class="flex min-h-screen bg-gray-100">
                ...
                <nav class="space-y-2">
                    ...
                    @foreach ($menuItems as $p)
                        @php
                            ...
                            $isActive = ($p['route'] != '#') && (Route::is($p['route']) || (str_contains($p['route'], 'news') && request()->routeIs('news.*')));
                        @endphp
        ...
        <main class="flex-1 p-8 overflow-y-auto">
            ...
            <div class="bg-[#38761D] p-6 rounded-lg shadow-md text-white">
                ...
                {{-- Tombol Delete --}}
                <form action="{{ route('news.destroy', $item) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus berita ini?');" class="mt-4">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-6 rounded transition duration-150">Hapus Berita</button>
                </form>
            </div>
        </main>
    </div>
